...
1. 当为`true`的时候：ws/http协议不检验是否以.flv为依据，进行协议解析。
...
1. 当为`true`的时候：ws/http协议不检验是否以`.fmp4`,`.mp4`为依据，进行协议解析。
...
1. 当为`true`的时候：ws/http协议不检验是否以`.mpeg4`为依据，进行协议解析。
...
1. 当为`true`的时候：ws协议不检验是否以`.h264`,`.h265`为依据，进行协议解析。
...
如果设置为`true`，则会在初始化解码器的时候，就会检查浏览器是否支持硬解码解码H265格式，如果不支持，就直接初始化wasm(simd)软解码。
...
8. close 是否显示关闭按钮
9. zoom 是否显示电子放大
10. scale 是否显示sacle按钮
11. performance 是否显示性能按钮
12. face 是否显示人脸识别按钮
12. fullscreenFn 绑定全屏按钮的自定义方法，如设置了，点击全屏按钮的时候触发。
15. fullscreenExitFn 绑定退出全屏按钮的自定义方法，如设置了，点击退出全屏按钮的时候触发。
13. screenshotFn 绑定截图按钮的自定义方法，如设置了，点击截图按钮的时候触发。
14. playFn 绑定播放按钮的自定义方法，如设置了，点击播放按钮的时候触发。
16. pauseFn 绑定暂停按钮的自定义方法，如设置了，点击暂停按钮的时候触发。
17. recordFn 绑定录制按钮的自定义方法，如设置了，点击录制按钮的时候触发。
18. recordStopFn 绑定停止录制按钮的自定义方法，如设置了，点击停止录制按钮的时候触发。
...
见[playback-api](./playback-api.md#playbackconfig)
...
见[playback-api](./playback-api.md#playbackforwardmaxratedecodeiframe)
...
如果配置了`true`,且decoder 是 `http(s)://cdn.com/deocder.js` 绝对地址。
...
decoder-pro.js 文件里面需要将`decoder-pro.wasm` 修改为`http(s)://cdn.com/decoder-pro.wasm`

decoder-pro-simd.js 文件里面需要将`decoder-pro-simd.wasm` 修改为`http(s)://cdn.com/decoder-pro-simd.wasm`

decoder-pro-audio.js 文件里面需要将`decoder-pro-audio.wasm` 修改为`http(s)://cdn.com/decoder-pro-audio.wasm`
...
> pro播放器不需要用户去判断`hasLoaded()`是否加载完毕 然后再执行 `play()` 方法了，播放器内部会自行判断。
...
- sampleRate：采样率
- channels：声道数
- encType：编码类型，包含：`AAC`，`ALAW`，`MULAW`

> 需要在触发`audioInfo`事件之后才能获取到
...
> 优先判断 `hasLoaded()` 方法， 因为`hasLoaded()`方法现在只会返回true，所以这个方法已经废弃。
...
1. channels：声频通道
2. sampleRate 采样率
3. encType 编码类型  支持 : `AAC`,`ALAW(G711-A)`,`MULAW(G711-U)`
...
```js
jessibucaPro.on("playFailedAndPaused", function (reason) {
    console.log('playFailedAndPaused',reason);
})
```
...
- 0: 表示卡顿 ； 当 `fps` 小于 `15` 的时候。
- 1: 表示流畅 ；当 `fps` 在 `15-24` 之间。
- 2: 表示非常流程； 当 `fps` 大于 `24` 的时候。
...
```js
jessibucaPro.on("videoSmooth", function (videoSmooth) {
    console.log("is video smooth", videoSmooth)
})
```
...
示例
```js
jessibucaPro.on(JessibucaPro.EVENTS.ptz, (event) => {
    console.log(event)
})
```
...
示例
```js
jessibucaPro.on(JessibucaPro.EVENTS.streamQualityChange, (event) => {
    console.log(event)
})
```
...
在点击`底部UI`上面的`x`按钮，会触发播放器close事件 ，内部会销毁播放器。
